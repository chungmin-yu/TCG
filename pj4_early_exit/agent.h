...
#define UNSTABLE_L 2
#define UNSTABLE_F 0.25
#define EARLYEXIT_P 0.75
...
public:
	...
	int MCTS(node* root, int count, int L){
		for (int i = 0; i < count; i++){
			...
			// Early Exit
			if (i > count/2 && i % 1000 == 0){
				int first_max = -1000000;
				int second_max = -1000000;
				...
				if (second_max + EARLYEXIT_P*(count-i) < first_max ){
					break;
				}
			}			
		...
		int visitValue = -1000000;
		float winRateValue = -1000000.0;
		int visitRes = -1;
		int winRateRes = -1;
		...
		if (L < UNSTABLE_L) {
			for(auto &childNode : root->children){
				int tmp1 = childNode->total;
				float tmp2 = childNode->calculateWinRate();
				if(tmp1 > visitValue){
					visitValue = tmp1;
					visitRes = childNode->pos; 
				}
				if(tmp2 > winRateValue){
					winRateValue = tmp2;
					winRateRes = childNode->pos; 
				}
			} 
			if (visitRes != winRateRes){
				L = L+1;
				visitRes = MCTS(root, (int)(UNSTABLE_F * count), L);
			}
		}else{
			for(auto &childNode : root->children){
				int tmp1 = childNode->total;
