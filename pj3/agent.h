...
public:
	...
	unsigned takeSimulation(node* Node){
		board b = Node->state;
		std::deque<int> dq;
		for(int i = 0; i < 81; ++i){
			dq.push_back(i);
	...
	void takeBackpropagation(std::vector<node*>& path, unsigned winner){
	...
	int MCTS(node* root, int count){
		for (int i = 0; i < count; i++){
			...
			// 3. Simualtion
			unsigned winner = takeSimulation(path.back());
			// 4. Backpropagation
			takeBackpropagation(path, winner);
		}
